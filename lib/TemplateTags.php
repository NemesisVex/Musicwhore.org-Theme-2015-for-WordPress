...
class TemplateTags {
		...
		<nav role="navigation">
			...
			<ul class="pager">
				<?php if ( is_attachment() ) : ?>
					<li><?php previous_post_link( '%link', __( 'Published In %title', WP_TEXT_DOMAIN ) ); ?></li>
				<?php else : ?>
					<li><?php previous_post_link( '%link', __( '<span title="Previous Post: %title">Previous</span>', WP_TEXT_DOMAIN ) ); ?></li>
					<li><?php next_post_link( '%link', __( '<span title="Next Post: %title">Next</span>', WP_TEXT_DOMAIN ) ); ?></li>
				<?php endif; ?>
			</ul>
		</nav><!-- .navigation -->
	<?php
	...
	public static function posted_on() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			$sticky = translate( 'Sticky', WP_TEXT_DOMAIN );
			echo <<< POSTED_ON
